...
extern void vunmap_range_noflush(unsigned long start, unsigned long end);
		 ...
		 struct pt_regs *regs)
{
	...
	list_for_each_entry_safe(p, tmp, &extender_mapped, list) {
		/*vunmap_range_noflush(p->addr, PAGE_SIZE);*/
		vunmap_range(p->addr, extender->windowed_size);
	...
	if (err) {
		vunmap_range_noflush(addr, extender->windowed_size);
		err = -ENOMEM;
		goto expdr_error;
	}
